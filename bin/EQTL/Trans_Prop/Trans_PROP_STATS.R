...
All <- rbind(TFs, CoF)
dim(final_trans_FDR_01)


# Annotation file
# Filter for genes/TFs which have gene information available

annotation_file <- read.csv(args[4]) %>% dplyr::select(1,2,3,6,7,8)
...
AF_trans <- read.csv(args[5], sep = "\t", header = F)
AF_random <- read.csv(args[6], sep = "\t", header = F)
...
snp_loc <- read.csv(args[7], sep="\t") %>% dplyr::select(1,2, 3)
...
# Gene location
gene_loc <- read.csv(args[8] ,sep = "\t") %>% dplyr::select(1,2,3,4) # get gene location
