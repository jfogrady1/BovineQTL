...
# Trans-eqtls 
final_trans_FDR_01 <- read.csv("Final_trans_FDR_01.txt", sep = ",")
...
# AF for subsampling
AF = read.csv("ALL_SNPS.header.fixed.frq", sep = "\t", header = F)
...
# SNP location
snp_loc <- read.csv("../../MatrixQTL/MQTL_inputs/SNP_location.txt", sep=",") %>% select(1,2, 3)
...
# Read in the data
Final_miRNA <-data.frame(matrix(ncol = 6, nrow = 0))
colnames(Final_miRNA) <- c("Distance", "Total_Random", "Total_Trans", "Input_SNPs", "Ratio_Random", "Ratio_Trans")
miRNA <- read.csv("TFs_MiRNA_Inputs/Bos_taurus_miRNAs.txt", skip = 13, sep = "\t", header = F)
miRNA$V1 <- sub("chr", "", miRNA$V1)
miRNA <- miRNA %>% select(1,3,4,5)
head(miRNA)
colnames(miRNA) = c("CHR", "Symbol", "Start_location", "End_location")
miRNA <- miRNA %>% filter(Symbol != "miRNA_primary_transcript")
miRNA$CHR <- as.integer(miRNA$CHR)
...
for (d in distance) {
  ...
  Master_trans <- dplyr::left_join(final_trans_FDR_01, miRNA, by = c("CHR_trans" = "CHR"))
  ...
  for (ele in 1:length(Random_final)) {
    ...
    Random_SNPs_final <- dplyr::left_join(Random_SNPs_final, miRNA, by = c("SNP_CHR"="CHR"))
    ...
    # Proportion of SNPs with a entity of interest nearby
...
df_long_simple = df_long %>% select(1,3)
...
# Write statistics
write.table(MASTER_STAT, file = "Results/miRNA_PROP_STATS.csv", row.names = F, col.names = T)
colnames(trans_df) <- colnames(df_long_simple)


# Write the RAW STATS TO A TABLE
df_long_simple = rbind(trans_df, df_long_simple)
write.table(df_long_simple, file = "Results/miRNA_Prop_RAW.txt", sep = "\t", row.names = F, col.names = T)
