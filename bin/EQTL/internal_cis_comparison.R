...
dim(plot_df) # 1524 - will be used to bind eferything
dim(plot_df_infected) # 497
dim(plot_df_control) # 1559
...
print(length(background_infected$ortholog_name))
...
print("Number of input genes for INFECTED only")
print(length(target$ortholog_name))

target_genes_infected <- list(target$ortholog_name)

results_infected <- gost(query = target_genes_infected,organism = "hsapiens", correction_method = "fdr", ordered_query = T, domain_scope = "known", custom_bg = background_infected$ortholog_name, user_threshold = 0.05, sources = c("GO:BP", "GO:CC", "KEGG", "REAC", "WP"), evcodes = T)
...
target <- gorth(query = plot_df_infected_all$phenotype_id, source_organism = "btaurus",  # Note, rownames of the res_df are our target
target_organism = "hsapiens", mthreshold = 1, filter_na = TRUE,
numeric_ns = "ENTREZGENE_ACC")
...
target_genes_infected <- list(target$ortholog_name)
results_infected_all <- gost(query = target_genes_infected,organism = "hsapiens", correction_method = "fdr", ordered_query = T, domain_scope = "known", custom_bg = background_infected$ortholog_name, user_threshold = 0.05, sources = c("GO:BP", "GO:CC", "KEGG", "REAC", "WP"), evcodes = T)
...
background_control <- gorth(query = background_control, source_organism = "btaurus", 
        target_organism = "hsapiens", mthreshold = 1, filter_na = TRUE,
        numeric_ns = "ENTREZGENE_ACC")
print("Number of background genes for control ALL")
print(length(background_control$ortholog_name))
...
target <- gorth(query = plot_df_control_all$phenotype_id, source_organism = "btaurus",  # Note, rownames of the res_df are our target
target_organism = "hsapiens", mthreshold = 1, filter_na = TRUE,
numeric_ns = "ENTREZGENE_ACC")
...
target_genes_control_all <- list(target$ortholog_name)
results_control_all <- gost(query = target_genes_control_all,organism = "hsapiens", correction_method = "fdr", ordered_query = T, domain_scope = "known", custom_bg = background_control$ortholog_name, user_threshold = 0.05, sources = c("GO:BP", "GO:CC", "KEGG", "REAC", "WP"), evcodes = T)
...
target <- gorth(query = plot_df_control$phenotype_id, source_organism = "btaurus",  # Note, rownames of the res_df are our target
target_organism = "hsapiens", mthreshold = 1, filter_na = TRUE,
numeric_ns = "ENTREZGENE_ACC")
...
target_genes_control <- list(target$ortholog_name)
results_control <- gost(query = target_genes_control,organism = "hsapiens", correction_method = "fdr", ordered_query = T, domain_scope = "known", custom_bg = background_control$ortholog_name, user_threshold = 0.05, sources = c("GO:BP", "GO:CC", "KEGG", "REAC", "WP"), evcodes = T)
...
plot <- gostplot(all_results, capped=FALSE, interactive=FALSE, pal=c(`GO:BP`="#ff9900", `GO:CC` ="purple", `KEGG`= "#dd4477",`REAC`="#3366cc",`WP`="violet")) + labs(y=bquote(~-log[10]~italic(P)[adj]), title = "", x = "Database") +
    theme_bw() +
    ylim(-1,-log10(0.0000000001)) +
    theme(axis.text.x = element_text(angle = 90, size = 12, vjust = 0.5, hjust =0.2, colour = "black"),
        ...
        legend.text = element_text(size=12)) + geom_text_repel(aes(label = all_results$result$term_name), colour = "black", max.overlaps = 40,
                  nudge_x = 0, nudge_y = 1.2, size = 2, fontface = "bold") + facet_wrap(~Group)
plot
...
background_DE <- gorth(query = ALL$phenotype_id, source_organism = "btaurus", 
        target_organism = "hsapiens", mthreshold = 1, filter_na = TRUE,
        numeric_ns = "ENTREZGENE_ACC")
...
target <- gorth(query = ALL_target_DE_genes$phenotype_id, source_organism = "btaurus",  # Note, rownames of the res_df are our target
target_organism = "hsapiens", mthreshold = 1, filter_na = TRUE,
numeric_ns = "ENTREZGENE_ACC")

print("Number of genes for control only")
print(length(target$ortholog_name)) # 869

target_genes_DE <- list(target$ortholog_name)
results_DE <- gost(query = target_genes_DE,organism = "hsapiens", correction_method = "fdr", ordered_query = T, domain_scope = "known", custom_bg = background_DE$ortholog_name, user_threshold = 0.05, sources = c("GO:BP", "GO:CC", "KEGG", "REAC", "WP"), evcodes = T)
...
lollipop_plot %>% 
  mutate(term_name = fct_reorder(term_name, desc(p_value))) %>% ggplot(., aes(-log10(p_value), term_name, colour = source)) +
        ...
        legend.title = element_text(size=14), #change legend title font size
        legend.text = element_text(size=12)) +
        scale_colour_manual(breaks = c("GO:BP", "GO:CC", "REAC", "WP"), values = c("#ff9900", "purple","#3366cc","violet")) +
        xlim(0,-log10(0.0001))
