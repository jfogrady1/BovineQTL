...
res_no_sva <- function(counts_in, coldata_in) {
    ...
    ddsMat <- DESeqDataSetFromMatrix(countData = counts_in,
                                    colData = coldata,
                                    design = ~ Batch + Genotype_PC1 + Genotype_PC2 + scale(Age) + Condition)
    ...
    test <- gorth(query = genes, source_organism = "btaurus", 
        target_organism = "hsapiens", mthreshold = Inf, filter_na = FALSE,
        numeric_ns = "ENTREZGENE_ACC")
    test$ortholog_name
    genes <- list(test$ortholog_name)

    target <- gorth(query = rownames(res_df), source_organism = "btaurus",  # Note, rownames of the res_df are our target
        target_organism = "hsapiens", mthreshold = Inf, filter_na = FALSE,
        numeric_ns = "ENTREZGENE_ACC")
    target_genes <- list(target$ortholog_name)
    head(target)
    results <- gost(query = genes,organism = "hsapiens", correction_method = "fdr", ordered_query = T, domain_scope = "known", custom_bg = target$ortholog_name, user_threshold = 0.05, sources = c("GO:BP", "GO:CC", "KEGG", "REAC", "WP"), evcodes = T)
    ...
    plot <- gostplot(results, capped=FALSE, interactive=FALSE, pal=c(`GO:BP`="#ff9900", `GO:CC` ="purple", `KEGG`= "#dd4477",`REAC`="#3366cc",`WP`="violet")) + labs(y=bquote(~-log[10]~italic(P)[adj]), title = "", x = "Database") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 0, size = 15, colour = "black"),
        axis.text.y = element_text(angle = 0, size = 15, colour = "black"),
        axis.title.x = element_text(size = 18, colour = "black"),
    ...
    scale_y_continuous(limits = c(0, 16), breaks = seq(0:16)) +
